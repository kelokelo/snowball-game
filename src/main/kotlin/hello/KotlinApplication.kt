...
class KotlinApplication {
    ...
    var moveList = ConcurrentLinkedQueue<String>()

    @Bean
    fun routes() = router {
        GET(fun(request: ServerRequest): Mono<out ServerResponse> {
            request.queryParam("move").ifPresent { move -> moveList.add(move) }
            return ServerResponse.ok().body(Mono.just("Let the battle begin! Get param: ${request.queryParam("move").orElse("nil")}" ))
        })
